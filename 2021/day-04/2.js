...
module.exports = input => {
  ...
  for (let i = 0; i < numbers.length; i += 1) {
    let winner;
    while (true) {
      let newWinner = getWinner(remainingBoards, numbers.slice(0, i));
      if (newWinner) {
        winner = newWinner;
        remainingBoards = remainingBoards.filter(board => board !== winner.board);
      } else {
        break;
      }
    }

    if (winner && remainingBoards.length === 0) {
